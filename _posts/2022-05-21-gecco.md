...
to learn parameter adaptation policies in a dedicated offline learning phase. Once a policy was learned, it can then be used to adapt an algorithms
parameter on a variety of problem instances. Still, the field is very young and there is much to learn.
In our recently accepted GECCO paper *[Theory-Inspired Parameter Control Benchmarks for Dynamic Algorithm Configuration](https://andrebiedenkapp.github.io/assets/pdf/22-gecco.pdf)*
...
The so created new solution candidate is compared to the old one. If it improves the performance it is kept, if not it is discarded.
...
Our code and data are publicly available at [https://github.com/ndangtt/LeadingOnesDAC](https://github.com/ndangtt/LeadingOnesDAC)
